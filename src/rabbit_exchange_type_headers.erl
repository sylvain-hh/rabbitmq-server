...
route(#exchange{name = XName},
    ...
    CurrentOrderedBindings = case mnesia:dirty_read(rabbit_headers_bindings, XName) of
        [] -> [];
...
% That has been done via add_binding/3
...
filter_and_order_match_operators([], Result) -> lists:keysort(1, Result);
% skip all x-* args..
filter_and_order_match_operators([ {<<"x-", _/binary>>, _, _} | Next ], CurResult) ->
    filter_and_order_match_operators (Next, CurResult);
% .. and copy the rest..
filter_and_order_match_operators([ Elem | Next ], CurResult) ->
    filter_and_order_match_operators (Next, [ Elem | CurResult]).


add_binding(transaction, #exchange{name = XName}, BindingToAdd = #binding{destination = MainDest, args = BindingArgs}) ->
% The binding Id is only useful to delete it from the new table via remove_bindings/3
    ...
    MatchOperators = filter_and_order_match_operators (BindingArgs, []),
    NewBinding = {BindingOrder, BindingId, BindingType, MainDest, MatchOperators},
    NewBindings = case mnesia:read (rabbit_headers_bindings, XName, write) of
        [] -> [NewBinding];
        [#headers_bindings{bindings = E}] -> lists:keysort(1, [NewBinding | E])
    end,
    NewRecord = #headers_bindings{exchange = XName, bindings = NewBindings},
    ok = mnesia:write (rabbit_headers_bindings, NewRecord, write);
add_binding(_, _, _) ->
    ok.


remove_bindings(transaction, #exchange{name = XName}, Bs) ->
    ...
    NewOrderedBindings = [ Bind || Bind={_,BId,_,_,_} <- CurrentOrderedBindings, lists:member(BId, BindingIdHashesToDelete) == false],
    NewRecord = #headers_bindings{exchange = XName, bindings = NewOrderedBindings},
    ok = mnesia:write (rabbit_headers_bindings, NewRecord, write);
