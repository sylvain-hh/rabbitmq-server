...
policy_validation1(_Config) ->
    ...
    SetPol = fun (Key, Val) ->
                     rabbit_ct_helpers:control_action_opts(
                       ["set_policy", "runtime_parameters-policy", ".*",
...
policy_opts_validation1(_Config) ->
    Set  = fun (Extra) -> rabbit_ct_helpers:control_action_opts(
                            ["set_policy", "policy_opts_validation-policy",
                             ".*", "{\"ha-mode\":\"all\"}"
                             | Extra]) end,
...
ha_policy_validation1(_Config) ->
    Set  = fun (JSON) -> rabbit_ct_helpers:control_action_opts(
                           ["set_policy", "ha_policy_validation-policy",
                            ".*", JSON]) end,
...
queue_master_location_policy_validation1(_Config) ->
    Set  = fun (JSON) ->
                   rabbit_ct_helpers:control_action_opts(
                     ["set_policy",
                      "queue_master_location_policy_validation-policy",
                      ".*", JSON] )
           end,
    OK   = fun (JSON) -> ok = Set(JSON) end,
    Fail = fun (JSON) -> {error_string, _} = Set(JSON) end,
...
queue_modes_policy_validation1(_Config) ->
    Set  = fun (JSON) ->
                   rabbit_ct_helpers:control_action_opts(
                     ["set_policy", "queue_modes_policy_validation-policy",
                      ".*", JSON] )
           end,
    OK   = fun (JSON) -> ok = Set(JSON) end,
    Fail = fun (JSON) -> {error_string, _} = Set(JSON) end,
    ...
    ok = rabbit_ct_helpers:control_action(clear_policy,
      ["queue_modes_policy_validation-policy"]),
    passed.
